...
# ─────────────────────────────────────────────────────────────────────────────
# GET DATA FROM SESSION STATE (from Fetch_Data page)
# ─────────────────────────────────────────────────────────────────────────────

if "fetched_data" not in st.session_state or st.session_state.fetched_data is None:
    st.warning(
        "📥 No data loaded. Please go to the **Fetch Data** page to upload or fetch PMS data."
    )
    with st.expander("ℹ️ How to get started"):
        st.markdown(
            """
1. Click **Fetch Data** in the sidebar
2. Upload a PMS export (CSV or Excel) OR fetch directly from JiBe
3. Come back to this **Analysis** page to view the dashboard
            """
        )
    st.stop()
...
if missing_cols:
    st.error(f"Loaded file is missing required column(s): {', '.join(missing_cols)}")
    st.stop()
